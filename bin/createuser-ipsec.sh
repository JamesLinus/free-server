...
if [[ ! -f ${ipsecSecFile} ]]; then
  echoS "IPSec config file (${ipsecSecFile}) is not detected. This you may not install it correctly. Exit." "stderr"
  sleep 2
  exit 1
fi
...
if [[ ! -z $(gawk "/^${username} / {print}" ${ipsecSecFile}) ]]; then
  echoS "Ooops, the user ${username} is exited in file ${ipsecSecFile} already. Exit" "stderr"
  sleep 2
  exit 0
fi
